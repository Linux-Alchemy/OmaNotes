...
A keyboard-first Markdown note editor for Omarchy. It is built on Qt 6 Widgets and KDE's
KTextEditor with its Vi mode, so editing is Vim and everything around the editor answers to
a Space leader. Notes are ordinary `.md` files in ordinary directories. A workspace is a
directory, not a vault: there is no database, no index file, and nothing to migrate.

It sits between Omawrite, which is a lovely place to write one thing, and Obsidian, which
organises a collection but keeps Vim inside the editor pane. OmaNotes is for people who
want a directory of notes, Vim across the whole application, and a window that looks like
it belongs on Omarchy.
