...
| `hostile-schemes.md` | `javascript:`, `data:` (link and image), `vscode://`, `ssh://`, `magnet:`, `mailto:`, a custom scheme, case and whitespace games | Every anchor but the `https` control refused with a status message naming the scheme; data: image shows a placeholder; the whitespace game never becomes an anchor at all. Rendered by `rendersTheHostileSchemesFixtureRefusingEveryLink` |
...
- **Pathological text** — a single multi-megabyte line, 2,000-row table,
  deeply nested blockquotes. Expected: rendering completes within the suite's
  timeout with the document's last marker present; no truncation exists and
  none is claimed. What follows the word "truncates" below is historical:
  rendering completes or truncates
