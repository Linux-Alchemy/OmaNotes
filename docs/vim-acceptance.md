...
| Insert-mode `Ctrl+V` is application paste, not Vim's literal-insert | Omarchy's universal Super+V delivers `Ctrl+V` (with a lingering Meta modifier from the held Super key); stock Vi mode would treat it as literal-insert and bare `p` never reads the system clipboard (LazyVim's `clipboard=unnamedplus` is why it does in Neovim) | Configure around | Matt chose mode-sensitive routing on 2026-09-07: `Ctrl+V` pastes the system clipboard in Insert mode only; Normal and Visual keep Vi's visual block. Application shortcut matching ignores the Meta modifier so the compositor-injected chord lands. `"+p`/`"+y` remain the Vim-correct clipboard registers everywhere |
