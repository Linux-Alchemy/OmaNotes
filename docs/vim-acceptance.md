...
| Insert-mode `Ctrl+V` is application paste, not Vim's literal-insert | Omarchy's universal Super+V delivers `Ctrl+V` (with a lingering Meta modifier from the held Super key); stock Vi mode would treat it as literal-insert and bare `p` never reads the system clipboard (LazyVim's `clipboard=unnamedplus` is why it does in Neovim) | Configure around | Matt's final call on 2026-09-07: the Super-injected chord (identified by its lingering Meta modifier) pastes in **any** mode — the universal promise Omarchy users expect, and the same behaviour their terminals give Neovim via bracketed paste. A bare `Ctrl+V` pastes only in Insert mode; Normal and Visual keep Vi's visual block. If Super is released before the injected key lands, the chord arrives Meta-less and falls back to the bare-key rules. `"+p`/`"+y` remain the Vim-correct clipboard registers everywhere |
