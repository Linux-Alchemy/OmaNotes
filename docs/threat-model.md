...
- Residue, bounded 2026-09-12: records for a workspace whose root no longer exists are kept
  seven days past the last write into its state directory, announced at launch with their
  location and expiry, then removed (ADR 0015, `SessionStore::sweepVanishedRoots`). Records for
  a root that exists are kept until that root is opened. Finding F-22, closed.
...
| F-22 | ~~Decide~~ **Closed 2026-09-12** | Recovery records accumulate without bound for dead workspaces. | T-P4 | Matt's ruling: a seven-day bound for roots that no longer exist, announced with location and expiry while it runs; roots that exist are kept. ADR 0015; `SessionStore::sweepVanishedRoots`, six store tests and two launch tests. |
